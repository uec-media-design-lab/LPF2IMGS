...
if settings.LPFMode == "MovingAvarage":
    MovingAverage()
...
# make output Movie
stream = fp.input(settings.outputDIR + "/output_%04d" + settings.EXT, framerate=settings.encodeFPS)
stream = fp.output(stream, settings.outputDIR + "/output.mp4",pix_fmt='yuv420p')
fp.run(stream)